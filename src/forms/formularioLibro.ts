...
// --- SETUP DEL FORMULARIO CON VERIFICACIÓN DE PERMISOS ---
function setupFormularioLibro() {
...
function generarCodigoBarras(idLibro: string): string {
...
async function exportarCodigoBarrasLibro(idLibro: string): Promise<string> {
  try {
    const db = await Database.load('sqlite:db_biblioteca.db');
